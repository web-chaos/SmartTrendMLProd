...
CONFIG = {
    ...
    "SIGNAL_SCORE_THRESHOLD": 8.5,               # Снижено: новый порог, основанный на пересмотренных баллах
    ...
    "STOCHASTIC_RANGE_LONG": [15, 78],           # Диапазон Stochastic для лонга
    "STOCHASTIC_RANGE_SHORT": [25, 88],          # Диапазон Stochastic для шорта
    # RSI параметры
...
#   Свинг-трейдинг (H4-D1)	FAST=12, SLOW=26, SIGNAL=9 (как сейчас)	Меньше шума, акцент на тренд.
    ...
    # === ПРОЧЕЕ ===
    "PER_SYMBOL_DELAY": 10.0,                    # Задержка между анализом монет (сек)
    ...
    "MARKET_ANALYSIS_SYMBOLS": [
        ...
        # === Metaverse / NFT ===
        "MOCAUSDT", "GHSTUSDT", "HYPEUSDT", "ZKUSDT", "XANUSDT", "PAXGUSDT",

        # === Индексные / прочее ===
        "SPXUSDT", "ETHFIUSDT", "PEOPLEUSDT", "DEGENUSDT",
        "CHILLGUYUSDT", "POPCATUSDT", "BIOUSDT", "PUMPUSDT", "ENSUSDT", "IPUSDT", "POLUSDT", "KASUSDT",

        # === Прочие (низкая ликвидность/неопределённые) ===
        "KERNELUSDT", "ARCUSDT", "GRIFFAINUSDT", "GASUSDT", "SKLUSDT", "BANDUSDT", "KAVAUSDT", "HAEDALUSDT", "KAITOUSDT", "CFXUSDT", "USELESSUSDT", "STXUSDT"
    ]
    ...
    try:
        for key in daily_stats:
            if key == "retracement_unique_symbols":
                daily_stats[key] = set()  # Особый случай для множества
            else:
                daily_stats[key] = 0
        bot.send_message(
            chat_id=message.chat.id,
...
async def market_analysis_loop(send_message, client, config):
    ...
    async def check_signal(symbol, df, df_htf, trend_global):
        try:
            ...
            # --- Определяем, может ли вмешаться ML ---
            ml_allowed = (
                final_signal is not None and 
                trend_global in ["long", "short"] and 
                final_signal == trend_global and
                sd_reason is None
            )

            # --- Обработка ML (если активен) ---
            ml_out = {}
            if ML_CONFIG.get("enabled", False) and ml_allowed:
                passed_by_bot = is_valid  # Используем уже вычисленное значение
                
                # ВЫЗОВ ML - ТОЛЬКО 2 ПАРАМЕТРА!
                ml_out = ml_handle_signal(filters_results, passed_by_bot)
                
                # СОХРАНЯЕМ ВСЕ ПОЛЯ ИЗ ML_RESULT
                if ml_out and "ml_result" in ml_out:
                    filters_results.update(ml_out["ml_result"])
                
                # Дополнительно сохраняем ml_allowed
                filters_results["ml_allowed"] = 1 if ml_allowed else 0
            else:
            ...
            final_should_send = is_valid  # по умолчанию решение бота

            if ml_allowed and ML_CONFIG.get("enabled", False):
                ...
                if mode == "advisory":
                    ...
                    # При neutral оставляем решение бота
                elif mode == "autonomous":
                    # Полностью автономный режим
                    final_should_send = (ml_action == "approve")
            ...
            if not final_should_send:
                if ML_CONFIG.get("enabled", False) and ml_allowed and ml_out.get("ml_action") != "neutral":
                    # Отклонено ML
                    ml_action = ml_out.get("ml_action", "neutral")
                    ml_reason = ml_out.get("ml_reason", "")
                    print(f"[ML FILTER] Сигнал {symbol} отклонён (режим: {mode}, действие: {ml_action}, причина: {ml_reason})")
                else:
                    # Отклонено базовой логикой бота
                    print(f"[BOT FILTER] Сигнал {symbol} отклонён | Score: long={score_long:.1f}/short={score_short:.1f}, threshold={score_threshold} | Trend: {trend_global} | S/D: {sd_reason}")

            #TODO: check_signal_end
            return {
                "signal_id": filters_results["signal_id"],
                "side": final_signal,
                "filters_results": filters_results,  # Добавьте это
                "ml_out": ml_out  # И это
            }
            ...
            for symbol in config["MARKET_ANALYSIS_SYMBOLS"]:
                try:
                    ...
                    if symbol not in active_trades:
                        ...
                        filter_map = {
                            "USE_ATR_FILTER": lambda x: f"ATR: <code>{x['atr']:.6f}</code>",
                            "USE_VOLUME_FILTER": lambda x: f"Объём: <code>{x['volume']:.2f}</code> {get_volume_arrow(x['volume'], x['vol_mean'])} (ср.: {x['vol_mean']:.2f})",
                            "USE_TRIX_FILTER": lambda x: f"TRIX: <code>{x['trix']:.6f}</code> {get_trix_arrow(x['trix'], x.get('prev_trix'))}",
                            "USE_SUPERTREND_FILTER": lambda x: f"ST: <code>{st_arrow_ltf}</code> / <code>{st_arrow_htf}</code>",
                            "USE_ADX_FILTER": lambda x: f"ADX: <code>{x['adx']:.2f}</code>",
                            "USE_MACD_FILTER": lambda x: f"MACD: <code>{x['macd']:.6f}</code> / {x['macd_signal']:.6f}",
                            "USE_STOCH_FILTER": lambda x: f"Stoch: <code>{x['stoch']:.2f}</code>",
                            "USE_RSI_FILTER": lambda x: f"RSI: <code>{x['rsi']:.2f}</code>",
                            "USE_EMA_FILTER": lambda x: f"EMA: <code>{x['ema_fast']:.4f}</code> / {x['ema_slow']:.4f}"
                        }

                        def build_filters_info(indicators, tf_name, symbol, side):
                            info = f"\n<b>Фильтры {tf_name}:</b>\n"
                            
                            # Пробегаем все фильтры и добавляем, если включены
                            for cfg_key, formatter in filter_map.items():
                                if CONFIG.get(cfg_key, False):
                                    info += formatter(indicators) + "\n"
                            
                            # CDV отдельно
                            if CONFIG.get("USE_CDV_FILTER", False):
                                ratio, vol = get_cdv_ratio(symbol, tf_name)
                                if ratio is not None:
                                    if side == "long":
                                        icon = "🟢 Buy" if ratio >= CONFIG["CDV_MIN_THRESHOLD"] else "🟡 Sell"
                                    else:
                                        icon = "🔴 Sell" if ratio <= -CONFIG["CDV_MIN_THRESHOLD"] else "🟡 Buy"
                                    info += f"CDV: {icon} {ratio:+.2%} (объём {vol:.2f})\n"
                            
                            return info

                        # Собираем фильтры для двух таймфреймов
                        filters_info = build_filters_info(ind, config['MARKET_ANALYSIS_TF'][0], symbol, side)
                        filters_info += build_filters_info(ind_htf, config['MARKET_ANALYSIS_TF'][1], symbol, side)
