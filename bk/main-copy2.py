...
async def market_analysis_loop(send_message, client, config):
    ...
    async def check_signal(symbol, df, df_htf, trend_global):
        try:
            ...
            # --- ML оценка (только для статистики, не влияет на валидность) ---
            ml_out = {}
            ...
            if ml_allowed:
                ...
                ml_out = ml_handle_signal(filters_results, is_valid)
                ...
                if ML_CONFIG.get("enabled", False):
                    ...
                    if mode == "advisory":
                        ...
                        if ml_action == "approve" and not is_valid:
                            print(f"[ML OVERRIDE] {symbol}: ML одобрила отклоненный сигнал")
                            is_valid = True
                        elif ml_action == "reject" and is_valid:
            ...
            if trend_global in ["long", "short"]:
                ...
                print(f"[DB] Сигнал {filters_results.get('signal_id')} записан — глобальный тренд определён")
            else:
