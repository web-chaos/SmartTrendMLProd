...
ML_FEATURE_ORDER1 = [
    ...
    "rsi_ltf", "rsi_htf", "rsi_prev_ltf", "rsi_prev_htf", "rsi_delta_ltf", "rsi_delta_htf", "rsi_long_score", "rsi_short_score",
    ...
    "stoch_long_passed", "stoch_short_passed",
...
ML_FEATURE_ORDER2 = [
    # === Топовые признаки по важности (основа модели) ===
    "cdv_ltf_value", "adx_htf_value", "trix_htf",
    "macd_momentum", "rsi_htf",
    "stoch_ltf", "volume_normalized_body", "final_scores",
    "macd_htf_diff", "volume_delta", "cdv_ltf_volume",
    "macd_htf_momentum", "volume_value", "adx_ltf_value",
    "trix_ltf", "macd_current_diff", "potential_value",
    "rsi_prev_htf", "atr_ltf_value", "rsi_ltf",
    "macd_prev_diff", "rsi_prev_ltf",
    "stoch_htf", "stoch_prev_htf", "stoch_prev_ltf",
    "volume_mean", "adx_ltf_score", "stoch_delta_htf",
    "volume_mean_long", "rsi_1h_long_score",
    "macd_1h_short_type", "atr_ltf_score",
    "adx_htf_score", "rsi_delta_ltf",

    # === RSI (детализация) ===
    "rsi_delta_htf", "rsi_long_score", "rsi_short_score",
    "rsi_1h_short_score", "rsi_4h_long_score", "rsi_4h_short_score",
    "rsi_1h_long_type", "rsi_1h_short_type", "rsi_4h_long_type", "rsi_4h_short_type",

    # === Stoch (детализация) ===
    "stoch_delta_ltf", "stoch_long_passed", "stoch_short_passed",

    # === MACD ===
    "macd_prev_htf_diff", "macd_long", "macd_short",
    "macd_1h_long_score", "macd_1h_short_score",
    "macd_4h_long_score", "macd_4h_short_score",

    # === TRIX / EMA ===
    "trix_long_score", "trix_short_score",
    "ema_diff_ltf", "ema_diff_htf", "ema_long_score", "ema_short_score",

    # === CDV (подробно) ===
    "cdv_htf_value", "cdv_ltf_score", "cdv_htf_score",
    "cdv_htf_volume", "cdv_ltf_normalized", "cdv_htf_normalized",
    "cdv_ltf_passed", "cdv_htf_passed",

    # === Потенциал и структура ===
    "potential_passed", "market_structure_ltf_score", "market_structure_htf_score",
    "market_structure_ltf_passed", "market_structure_htf_passed",
    "structural_score", "structural_mode_config",

    # === Свечные паттерны и бонусы ===
    "candle_pattern_score", "candle_pattern_passed",
    "candle_pattern_sd_bonus", "candle_pattern_sd_bonus_score", "candle_pattern_sd_bonus_passed",
    ...
    # === Прочие фильтры ===
    "atr_ltf_passed", "adx_ltf_passed", "adx_htf_passed",
    "rsi_long_passed", "rsi_short_passed",
    "volume_passed", "ema_long_passed", "ema_short_passed",
    "trix_long_passed", "trix_short_passed",

    # === Таймфреймные категориальные фичи ===
    "trend_global", "adx_mode_config", "supertrend_mode", "sd_zone_type"
]
...
def main():
    ...
    for col in categorical_cols:
        if col in available_feats:
            ...
            mapping = {str(k): int(v) for k, v in zip(list(le.classes_), list(le.transform(le.classes_)))}
    ...
    model3 = lgb.LGBMClassifier(
        objective="multiclass",
        num_class=4,  # теперь 4 класса: 0=STOP, 1=TP_LOW, 2=TP_MEDIUM, 3=TP_HIGH
        n_estimators=300,  # меньше деревьев
        learning_rate=0.1,
        max_depth=5,  # меньшая глубина
        num_leaves=20,
        subsample=0.7,
        colsample_bytree=0.7,
        reg_alpha=0.5,  # добавить L1 регуляризацию
        reg_lambda=0.5,  # добавить L2 регуляризацию
        class_weight="balanced",
        random_state=RANDOM_STATE,
        n_jobs=-1
    )
    ...
    model4 = lgb.LGBMClassifier(
        objective="multiclass",
        num_class=4,   # 0=STOP, 1=TP_LOW, 2=TP_MEDIUM, 3=TP_HIGH
        n_estimators=80,
        learning_rate=0.15,
        max_depth=3,
        num_leaves=8,
