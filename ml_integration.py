...
def analyze_signal_row(filters_results: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        X = pd.DataFrame([row]).apply(pd.to_numeric, errors='coerce').fillna(0).astype(float)
    ...
    if ml_class == 0:
        ...
        if tps_display:
            ...
            forecast_text = (
                ...
                f"<code>{entry_low:.4f}</code>-<code>{entry_high:.4f}</code>, после подтверждения — движение к целям TP1–TP{ml_class} ({tps_text})."
            )
        else:
...
def generate_signal_text(
    ...
    if strength < 40:
        ...
        desc, emoji = "нейтральный ", "🟠"
    elif strength < 80:
        desc = "средне-сильный " if signal_type=="long" else "средне-сильный "
        emoji = "🟢" if signal_type=="long" else "🔴"
    else:
        desc = "мощный импульсный" if signal_type=="long" else "мощный импульсный"
        emoji = "🟣"

    ema_fast_ltf = filters_results.get("ema_fast")
    atr_ltf = filters_results.get("atr_ltf_value")
    # --- 2. Диапазон входа ---
    if strength >= 60 and entry_zone_min is not None and entry_zone_max is not None:
        entry_low, entry_high = entry_zone_min, entry_zone_max
    elif ema_fast_ltf is not None:
        # Для слабого сигнала используем fast EMA ± ATR если есть
        entry_low = ema_fast_ltf - (atr_ltf * 0.5 if atr_ltf else 0)
        entry_high = ema_fast_ltf + (atr_ltf * 0.5 if atr_ltf else 0)
    else:
        entry_low = entry_high = 0.0  # fallback
    ...
    if ml_class == 0:
        ...
        if tps_display:
            tps_text = f"<code>{tps_display[0]:.4f}</code>"
            if len(tps_display) > 1:
                tps_text += f" → <code>{tps_display[-1]:.4f}</code>"
        else:
    ...
    # --- 6. Формируем текст ---
    message = (
        f"🔮 Сила: <code>{strength}%</code> ({desc} {emoji})\n"
        f"🎯 Вход: <code>{entry_low:.4f}</code>–<code>{entry_high:.4f}</code>\n"
        f"🏁 Цели: {tps_text}\n"
